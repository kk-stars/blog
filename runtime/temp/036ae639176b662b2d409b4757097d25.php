<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:55:"D:\B\AppServ\www\blog/application/index\view\re\re.html";i:1524128601;}*/ ?>
<?php
...
<head>
  ...
  <title>BABYSBREATH 满 天 星  ☆  何 坤 个 人 博 客  - 注册</title>
...
<div class="log">
  ...
  <div class="am-u-lg-3 am-u-md-6 am-u-sm-8 am-u-sm-centered log-content">
    ...
    <br>
    <form class="am-form" id="log-form" method="post" action="<?php echo url('re/sms'); ?>">
      <div class="am-input-group am-animation-slide-left log-animation-delay">
        <input type="text" id="mobile" class="am-form-field am-radius log-input" name="mobile" placeholder="请输入你的手机电话" minlength="11" size="25" data-validation-message="请输入你的手机电话" onkeyup="this.value=this.value.replace(/\s+/g,'')" required>
        <span class="am-input-group-label log-icon am-radius">
        <i class="am-icon-phone am-icon-sm am-icon-fw"></i></span>
      </div>
      <br>
      ...
      <div class="am-input-group am-radius am-animation-slide-left">
        <input type="text" id="doc-vld-email-2-1" class="am-radius" name="userName" data-validation-message="请设置您的用户名" placeholder="请设置您的用户名" onkeyup="this.value=this.value.replace(/\s+/g,'')" required/>
        <span class="am-input-group-label log-icon am-radius">
        <i class="am-icon-user am-icon-sm am-icon-fw"></i></span>
      </div>
      <br>

      <div class="am-input-group am-animation-slide-left log-animation-delay">
        <input type="password" id="log-password" class="am-form-field am-radius log-input" name="userPassword" placeholder="请设置用户密码(长度8位)" onkeyup="this.value=this.value.replace(/\s+/g,'')" minlength="8" required>
        <span class="am-input-group-label log-icon am-radius"><i class="am-icon-lock am-icon-sm am-icon-fw"></i></span>
      </div>
      <br>

      <div class="am-input-group am-animation-slide-left log-animation-delay-a">
        <input type="password" data-equal-to="#log-password" id="reUserPassword" class="am-form-field am-radius log-input" name="reUserPassword" onkeyup="this.value=this.value.replace(/\s+/g,'')" placeholder="确认密码" data-validation-message="请确认密码一致" required>
        <span class="am-input-group-label log-icon am-radius"><i class="am-icon-lock am-icon-sm am-icon-fw"></i></span>
      </div>
      <br>

      <button type="button" class="am-btn am-btn-primary am-btn-block am-btn-lg am-radius am-animation-slide-bottom log-animation-delay-b" id="submit">注 册</button>
      <br>
...
$(function(){
	$('body').keyup(function(event){
		if(event.keyCode==13){ 	 //回车键  13
			$('#submit').click();//回车键触发事件
		}
	});
});

$('#submit').click(function(){
    var uName = $('#doc-vld-email-2-1').val();		/*	 val 获取管理员名称输入框的 value 值	*/
	var uPwd = $('#log-password').val();
	var rPwd = $('#reUserPassword').val();
	var uMobile = $('#mobile').val();
	var uChcode = $('#check-code').val();
    var style = {							/*   style 错误提示框样式  */
       	position: "fixed",
       	top: "0",
       	marginLeft:"45%",
        background:"#fff",
    ...
    if(uPwd !== '' && rPwd !== '' && rPwd === uPwd){
	    $.post('__URL__/re',{'mobile':uMobile,'mobile_code':uChcode,'userName':uName,'userPassword':uPwd,'repeatPassword':rPwd},function(result){
	...
	function get_mobile_code(){
        $.post('__URL__/sms', {mobile:jQuery.trim($('#mobile').val()),send_code:<?php echo $_SESSION['send_code'];?>}, function(msg) {
			...
			if(msg=='提交成功'){
	            ...
	            alert(jQuery.trim(unescape(d.message)));
			}
        });
	};
	var iTime = 59;
