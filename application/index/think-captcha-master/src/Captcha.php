...
class Captcha
{
    ...
    public function entry($id = '')
    {
        ...
        //$ttfPath = '../assets/ttfs/AdobeHeitiStd-Regular.otf';

        /* if (empty($this->fontttf)) {
            $dir  = dir($ttfPath);
            $ttfs = [];
            while (false !== ($file = $dir->read())) {
                if ('.' != $file[0] && substr($file, -4) == '.otf') {
                    $ttfs[] = $file;
                }
            }
            $dir->close();
    ...
    private function writeNoise()
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->im, 4, mt_rand(-10, $this->imageW), mt_rand(-10, $this->imageH), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
